...
class Tree:
    ...
    def visualize(self, layer_num=5):
        """
        Visualizes the tree structure up to a specified number of layers.
        
        Parameters:
        - layer_num: The number of layers to visualize.
        """
        graph = nx.DiGraph()

        def build_graph(node, depth=0, pos=0, layer_positions=None):
            """Recursively add nodes and edges to the graph for visualization."""
            if layer_positions is None:
                layer_positions = {}

            # Position nodes horizontally based on their depth and position in the layer
            layer_positions[(depth, pos)] = node.id
            graph.add_node(node.id, label=f"Node {node.id}")

            for i, child in enumerate(node.children):
                child_pos = pos * len(node.children) + i  # Adjust position for each child
                graph.add_edge(node.id, child.id)  # Add edge between parent and child
                build_graph(child, depth + 1, child_pos, layer_positions)

            return layer_positions

        # Build graph and retrieve node positions
        layer_positions = build_graph(self.root)

        # Calculate positions for each node in matplotlib
        pos = {node: (x, -y) for (y, x), node in layer_positions.items() if node in graph.nodes}
        labels = nx.get_node_attributes(graph, 'label')

        # Check for any missing nodes and handle them by assigning a default position
        missing_nodes = set(graph.nodes) - set(pos.keys())
        if missing_nodes:
            print("Warning: Some nodes are missing positions:", missing_nodes)
            for i, missing_node in enumerate(missing_nodes):
                pos[missing_node] = (0, -i)  # Assign each missing node a unique default position

        # Plotting the graph using matplotlib
        plt.figure(figsize=(15, 10))
        nx.draw(graph, pos, with_labels=True, labels=labels, node_size=800, node_color="skyblue", font_size=8, font_weight="bold", font_color="black")
